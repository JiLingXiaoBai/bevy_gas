...
1. 通过 `AttributeIdRegister::request_or_register_attribute_id("MyAttribute")` 注册
2. 在 `AttributeSet::initialize_attribute(id, base_value, executor, clamp)` 中初始化
3. 通过返回的 `AttributeId` 引用
