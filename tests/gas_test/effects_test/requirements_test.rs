...
fn queued_immunity_is_visible_to_the_next_request() {
...

    {
        ...
        queue
            .push_application(target, immunity_effect, EffectPayload::new(source, None, 1))
            .unwrap();
        queue
            .push_application(target, incoming, EffectPayload::new(source, None, 1))
            .unwrap();
    }
...
fn requirement_converges_between_two_gameplay_requests() {
...

    {
        let mut queue = app.world_mut().resource_mut::<GameplayExecutionQueue>();
        queue
            .push_application(target, disable_effect, EffectPayload::new(source, None, 1))
            .unwrap();
        queue
            .push_application(target, incoming, EffectPayload::new(source, None, 1))
            .unwrap();
